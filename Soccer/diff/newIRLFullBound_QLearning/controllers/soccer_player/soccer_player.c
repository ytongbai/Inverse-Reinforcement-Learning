    ...
    if(ctrInput > 1){
      ...
      if(buffer[6]<0.5){ // 判斷episode
        ...
        if(buffer[4]<0.5){ // 判斷ball有沒有動
          if(state[0]==bState[0] && state[1]==bState[1]){ //判斷是否在同一個狀態
            ctrWall=ctrWall+1;
            
            if(ctrWall > 30*(bState[0]+12)){ //若在同一個狀態太久，視為撞牆
